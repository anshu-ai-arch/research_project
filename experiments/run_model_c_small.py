...
def run_experiment_c(num_epochs: int = 20):
    device = get_m1_device()
    print("\n=================================================================")
    print("      EXPERIMENT C — SMALL CNN (32 -> 64 -> 64 | ~164K Params)    ")
    print("=================================================================")
    print(f"[*] Target Acceleration Device: {device}")
    print(f"[*] CNN Architecture: Conv1D(1, 32, 5) -> Conv1D(32, 64, 5) -> Conv1D(64, 64, 3)")
    ...
    best_val_acc = 0.0
    checkpoint_path = Path("checkpoints/model_c_small_augmented_best.pth")
    ...
    print(f"\n[✓] Experiment C Training Complete! Best Val Acc: {best_val_acc:.2f}%")
    ...
    results_file = Path("experiments/results_experiment_c.json")
    save_summary = {
        "model_name": "Experiment C (Small CNN)",
        "trainable_params": trainable_params,
    ...
    print(f"[✓] Saved Experiment C evaluation results to '{results_file}'")
    return save_summary


if __name__ == "__main__":
    run_experiment_c(num_epochs=20)
