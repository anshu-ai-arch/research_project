...
def run_experiment_d(num_epochs: int = 20):
    device = get_m1_device()
    print("\n=================================================================")
    print("      EXPERIMENT D — MEDIUM CNN + BiGRU32 (hidden=32 | ~150K Params)")
    print("=================================================================")
    print(f"[*] Target Acceleration Device: {device}")
    print(f"[*] CNN Architecture: Conv1D(1, 64, 5) -> Conv1D(64, 128, 5) -> Conv1D(128, 128, 3)")
    print(f"[*] BiGRU Architecture: 2-layer BiGRU, hidden_size=32 per direction")
    ...
    best_val_acc = 0.0
    checkpoint_path = Path("checkpoints/model_c_medium_gru32_augmented_best.pth")
    ...
    print(f"\n[✓] Experiment D Training Complete! Best Val Acc: {best_val_acc:.2f}%")
    ...
    results_file = Path("experiments/results_experiment_d.json")
    save_summary = {
        "model_name": "Experiment D (Medium CNN + BiGRU32)",
        "trainable_params": trainable_params,
    ...
    print(f"[✓] Saved Experiment D evaluation results to '{results_file}'")
    return save_summary


if __name__ == "__main__":
    run_experiment_d(num_epochs=20)
